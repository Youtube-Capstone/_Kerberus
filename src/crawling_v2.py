...
class crawling:
    ...
    def __getvideo(self, video_url):
        ...
        primary = video_html.find('ytd-video-primary-info-renderer',
                                       {'class': 'style-scope ytd-watch-flexy'})
        secondary = video_html.find('ytd-video-secondary-info-renderer',
                                         {'class': 'style-scope ytd-watch-flexy'})
        comments = video_html.find('ytd-item-section-renderer',
                                        {'class': 'style-scope ytd-comments'})

        video_driver.quit()
        print("finish getvideo")

        return primary, secondary, comments

    def __immutabilityvideoinformation(self,secondary):
        print("start immutabilityvideoinformation")

        upload_date_text = secondary.find('span',
        ...
        channel_name = secondary.find('a', {'class': 'yt-simple-endpoint style-scope yt-formatted-string'}).text
        channel_url = 'https://www.youtube.com' + \
                      secondary.find('a', {'class': 'yt-simple-endpoint style-scope yt-formatted-string'})['href']

        video_content_text = secondary.find('yt-formatted-string', {
            'class': 'content style-scope ytd-video-secondary-info-renderer'}).text

        video_content = self.__replace(video_content_text)

        find_category = secondary.find('div', {'id': 'collapsible'})
    ...
    def __variabilityvideoinformation(self, primary, secondary):
        print("start variabilityvideoinformation")
        view_text = primary.find('span', {'class': 'view-count style-scope yt-view-count-renderer'}).text
        view = self.__extraction(view_text)
        like_or_dislike = primary.find_all('yt-formatted-string',
        ...
        channel_subscribe_count_text = secondary.find('div', {'id': 'subscribe-button'})
        channel_subscribe_count = self.__extraction(
            channel_subscribe_count_text.find('yt-formatted-string', {'id': 'text'}).text)

        video_title_text = primary.find('yt-formatted-string',
    ...
    def __getcomment(self,comments):
        comment = []
        comment_count = 0
        print("start getcomment")
        comment_count_check = comments.find('yt-formatted-string',
                                                   {
        ...
        video_comment_text = comments.find_all('yt-formatted-string',
                                                      {'id': 'content-text'})  # 한 동영상에 대한 모든 댓글 태그 가져옴
        video_comment_writer = comments.find_all('a',
                                                        {'id': 'author-text'})
    ...
    def get_youtube_information_comments(self,URL,CONFIRM):
        try:
            PRIMARY, SECONDARY, COMMENTS = self.__getvideo(URL)
            print("exist video")
            if CONFIRM == False:
                COMMENT_COUNT, COMMENT = self.__getcomment(COMMENTS)
                if COMMENT_COUNT == 0:
                    return -1
                else:
                    UPLOAD_DATE_FORMAT, CHANNEL_NAME, CHANNEL_URL, VIDEO_CONTENT, VIDEO_CATEGORY = self.__immutabilityvideoinformation(
                        SECONDARY)
                    VIEW, LIKE, DISLIKE, CHANNEL_SUBSCRIBE_COUNT, VIDEO_TITLE = self.__variabilityvideoinformation(
                        PRIMARY, SECONDARY)
                    return [UPLOAD_DATE_FORMAT, CHANNEL_NAME, CHANNEL_URL, VIDEO_CONTENT, VIDEO_CATEGORY, VIEW, LIKE, DISLIKE, CHANNEL_SUBSCRIBE_COUNT, VIDEO_TITLE, COMMENT_COUNT, COMMENT]

            else:
                VIEW, LIKE, DISLIKE, CHANNEL_SUBSCRIBE_COUNT, VIDEO_TITLE = self.__variabilityvideoinformation(PRIMARY,
                                                                                                               SECONDARY)
                COMMENT_COUNT, COMMENT = self.__getcomment(COMMENTS)
                return [VIEW, LIKE, DISLIKE, CHANNEL_SUBSCRIBE_COUNT, VIDEO_TITLE, COMMENT_COUNT, COMMENT]
        except:
            return -1
